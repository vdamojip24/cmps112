...
let rec print_number number = 
    if (String.length (string_of_bigint number)) >= 70
        then (printf "%s\\\n%!" (String.sub 
          (string_of_bigint number) 0 69);
        print_number (bigint_of_string (String.sub 
          (string_of_bigint number) 69 
            (String.length (string_of_bigint number) - 69))))
    else printf "%s\n%!" (string_of_bigint number)
...
let executereg (thestack: stack_t) (oper: char) (reg: int) =
    try match oper with
        | 'l' -> (let entry = aget symbol_table reg
            in match entry with
